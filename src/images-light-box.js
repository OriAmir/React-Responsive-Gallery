...
export const ImagesLightBox = ({
  imagesLightbox, photoIndex, lightBoxDispatch, lightBoxAdditionalProps
}) => (
  <Lightbox
    mainSrc={imagesLightbox[photoIndex].src}
    nextSrc={imagesLightbox[(photoIndex + 1) % imagesLightbox.length].src}
    prevSrc={imagesLightbox[(photoIndex + imagesLightbox.length - 1)
     % imagesLightbox.length].src}
    onCloseRequest={() => lightBoxDispatch({ type: 'close' })}
    onMovePrevRequest={() => lightBoxDispatch({
      type: 'photoIndex',
      photoIndex: (photoIndex + imagesLightbox.length - 1) % imagesLightbox.length
    })}
    onMoveNextRequest={() => lightBoxDispatch({
      type: 'photoIndex',
